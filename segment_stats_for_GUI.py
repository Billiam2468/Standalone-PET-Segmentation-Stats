...
def more_optimized_suv_statistics(roi_data, pet_data, reference, pet_file):
    ...
    #HU_values = calculate_HU_stats(roi_data, ct_array, reference)
    #print(HU_values)
    ...
    for roi in unique_rois:
        ...
        if suv_values.size > 0:
            suv_stats[reference[int(roi)]] = {
                'SUV_mean': np.mean(suv_values) * SUV_factor,
                'SUV_max': np.max(suv_values) * SUV_factor,
                'SUV_min': np.min(suv_values) * SUV_factor,
                'SUV_std_dev': np.std(suv_values) * SUV_factor,
                'SUV_median': np.median(suv_values) * SUV_factor,
...
def calculate_HU_stats(seg_dir, reference, pet_file):
    ...
    resampler.SetReferenceImage(dicom_image)  # Use PET as reference for spacing and size
    ...
    mask = flat_roi_data > 0
    flat_roi_data = flat_roi_data[mask]
    flat_ct_data = flat_ct_data[mask]
...
# Generates NIFTI for each PET DICOM directory along with storing metadata necessary for calculating SUVs
...
def get_SUV_factor_from_PET_NIFTI(pet_nifti):
    ...
    # Extracting DICOM directory
    extensions = nifti_file.header.extensions
    dicom_folder = ""
    for ext in extensions:
        if ext.get_code() == 6:  # Code 6 matches the one used to store dicom_folder
            dicom_folder = ext.get_content().decode('utf-8')  # Convert bytes to string
    ...
    return SUV_factor, dicom_folder
    ...
    while True:
        ...
        if event == "Exit" or event == sg.WINDOW_CLOSED:
            ...
            try:
                ...
                SUV_stats, HU_stats = statistics_from_rois(seg_dir, nifti_output_dir, name_reference=names)

                SUV_csv_path = os.path.join(csv_output_dir, "SUV_" + csv_name + ".csv")
                HU_csv_path = os.path.join(csv_output_dir, "HU_" + csv_name + ".csv")

                save_to_csv(SUV_stats, SUV_csv_path)
                save_to_csv(HU_stats, HU_csv_path)
